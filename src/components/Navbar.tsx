  ...
  return (
    ...
    >
      <Link to="/" className="relative group flex items-center gap-3">
        ...
        />
        <span className={`text-2xl font-black tracking-tight transition-colors duration-500 ${isDarkBackground ? "text-white" : "bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 bg-clip-text text-transparent"}`}>
          FOCSERA
        </span>
      ...
      <div className="hidden lg:flex items-center gap-1">
        {mainMenu.map((item) =>
          item.type === "dropdown" ? (
            ...
            >
              <button
                className={`flex items-center gap-1 relative font-semibold text-sm px-4 py-2 rounded-full transition-all duration-500 ${textColor} ${hoverTextColor}`}
