...
import { Package, Palette, TrendingUp, ArrowRight, QrCode, X, Sparkles, Clock, MapPin, Utensils, Monitor } from 'lucide-react';
...
export default function ProductServices() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Submit via RPC (Handles both DB Insert + Email Trigger)
      console.log("Attempting submission via RPC...");

      const { data, error: rpcError } = await supabase.rpc('submit_dineqr_demo', {
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {/* Product 1: FocseraDineQR (Cinematic Dark Theme - ISOLATED) */}
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-auto">
                      <Button
                        ...
                        className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-black font-bold rounded-xl py-6 text-lg shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all duration-300 transform hover:-translate-y-1"
                      >
                        Book Demo
                      </Button>
                      <Button
                        type="button"
                        variant="outline"
                        className="flex-1 border-white/10 hover:border-emerald-500/50 text-gray-300 hover:text-white rounded-xl h-auto py-6 font-medium bg-transparent hover:bg-white/5 backdrop-blur-sm"
                      >
                        View More
                      </Button>
                    </div>
                ...
                {/* Product 2: Coming Soon (Light Theme Original) */}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden text-white flex flex-col max-h-[90vh]"
            >
              {/* Decorative top glow */}
              ...
              <div className="p-8 pb-6 border-b border-white/5 flex items-center justify-between relative z-10 flex-shrink-0">
                <div>
                  <h3 className="text-2xl font-serif font-medium tracking-wide text-white">
                    Book <span className="text-emerald-400">Demo</span>
                  </h3>
                  <p className="text-sm text-gray-400 mt-1">Experience the future of dining</p>
                </div>
                <button
                  onClick={handleClose}
              ...
              {isSuccess ? (
                <div className="p-12 flex flex-col items-center justify-center text-center space-y-6 relative z-10 h-96">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 max-w-sm mx-auto">
                      Thank you for your interest. A confirmation email has been sent to <span className="text-emerald-400">{formData.email}</span>.
                    </p>
                    <p className="text-gray-500 text-sm mt-4">We will be in touch shortly.</p>
                  </div>

                  <Button
                    onClick={handleClose}
                    className="mt-4 bg-white/10 hover:bg-white/20 text-white rounded-full px-8 py-2 border border-white/10"
                  >
                    Close
                  </Button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 pt-6 space-y-5 relative z-10 overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-4">
                    <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Contact Details</h4>
                    <div className="grid grid-cols-2 gap-5">
                      <div className="space-y-2">
                        <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Your Name</label>
                        <input
                          required
                          type="text"
                          className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all placeholder:text-gray-600 text-white"
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Phone</label>
                        <input
                          required
                          type="tel"
                          className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all placeholder:text-gray-600 text-white"
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Email</label>
                      <input
                        required
                        type="email"
                        className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all placeholder:text-gray-600 text-white"
                  ...
                  <div className="space-y-4">
                    <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Restaurant Details</h4>
                    <div className="grid grid-cols-2 gap-5">
                      <div className="space-y-2">
                        <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Restaurant Name</label>
                        <input
                          required
                          className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all placeholder:text-gray-600 text-white"
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Cuisine Type</label>
                        <div className="relative">
                          <Utensils className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
                          <input
                            className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all placeholder:text-gray-600 text-white"
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      <div className="space-y-2">
                        <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Tables</label>
                        <select
                          className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all text-gray-300"
                          value={formData.tableCount}
                          onChange={(e) => setFormData({ ...formData, tableCount: e.target.value })}
                        >
                          <option value="" className="bg-gray-900">Select...</option>
                          <option value="1-10" className="bg-gray-900">1-10 Tables</option>
                          <option value="11-30" className="bg-gray-900">11-30 Tables</option>
                          <option value="30+" className="bg-gray-900">30+ Tables</option>
                        </select>
                      </div>
                      <div className="space-y-2">
                        <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Current POS</label>
                        <div className="relative">
                          <Monitor className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={16} />
                          <input
                            className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all placeholder:text-gray-600 text-white"
                            placeholder="e.g. Petpooja, None"
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-medium text-emerald-500/80 uppercase tracking-wider">Address / Location</label>
                      <div className="relative">
                        <MapPin className="absolute left-4 top-3 text-gray-500" size={16} />
                        <textarea
                          rows={2}
                          className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:border-emerald-500/50 focus:bg-white/10 outline-none transition-all resize-none placeholder:text-gray-600 text-white"
                  ...
                  <div className="pt-4 pb-2">
                    <Button
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-500 text-white text-lg h-14 rounded-xl font-medium shadow-lg shadow-emerald-900/20 transition-all transform active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                    >
      ...
      <section className="py-24 bg-[#ECECEC]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl font-bold text-gray-900 mb-6">Ready to Launch Your Product?</h2>
          <p className="text-xl text-gray-600 mb-8">
            Let's collaborate to design, develop, and market products that stand out in the marketplace.
          </p>
          <Link
            to="/contact"
